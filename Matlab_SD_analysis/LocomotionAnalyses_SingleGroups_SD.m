...
day0 = '5/11/2024'; % start date, no "0" in date!
day1 = '5/12/2024'; % Lights on at 9am (for ctrl); Run ends 11pm this day
...
% day 0. start of experiment
frame_day0 = strfind(timeArrayCell, (strcat(day0,'5:00:00 PM')));
...
frame_night2 = strfind(timeArrayCell, (strcat(day1,'5:00:00 PM')));
...
%frame_mean = (frame_day1-frame_night1)/(24*60*60); %seconds in a day
frame_mean = (frame_night2-frame_day0)/(24*60*60); 
%frame_mean = (frame_testphase-frame_day0)/(24*60*60); 
...
%d96_clip = d96(frame_day0:frame_testphase,:);
%% 6. Bin activity/
...
ctrl = [1:48]; %96: number of wells. Only use if none of the fish should be excluded
%ctrl(ctrl==4) = [];
%ctrl(ctrl==13) = [];
allWells = [1:48]; %96: number of wells
...
%% 8.2 BINARY SLEEP PLOT
% binary sleep plot plots sleep over time
% black: fish were asleep (no activity (zeros) in 1 minute bin)
% white: fish were active (activity (non-zeros) in 1 minute bin)

fct_Binary_activity(a_clip);
